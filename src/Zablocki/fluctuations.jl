...
function pressure_fluctuation(T, mu, param; reltol=1e-2)
    integrand(x) = integrand_pressure(T, mu, x[1], x[2], param)
    return integrate(integrand, [0.0, 0.0], [sqrt(5) * param.Λ, param.Λ], reltol=reltol)
end
...
function pressure_fluctuation(trange::AbstractArray, mu, param)
    ...
    Threads.@threads for i in eachindex(trange)
        pres[i] = pressure_fluctuation(trange[i], 0.0, param)[1]
    end
...
function phase_zero(T, mu, ω, q, param)
    ...
    return Zablocki.phase_shift_sigma_q(T, mu, sqrt(ω^2 - q^2), 0.001, param)
end
...
function pressure_fluctuation_boosted(T, mu, param)
    ...
    return PhaseGN.hcubature(integrand, [0.0, 0.0], [sqrt(5) * param.Λ, param.Λ], reltol=1e-2)
end

function pressure_fluctuation_boosted(trange::AbstractArray, mu, param)
    pressure_boost = zeros(length(trange2))
    @showprogress Threads.@threads for i in eachindex(trange2)
        pressure_boost[i] = pressure2(phase_zero, trange2[i], 0.0, param)[1]
    end
