...
function ckpk(k, p, m, e, ep, theta)
    return 1 + e * ep * ((k * p * cos(theta) + k^2) + m^2) / (Ekk(k, m) * Ekk(sqrt(k^2 + p^2 + 2k * p * cos(theta)), m))
end

function integrand_Pi_0_1(T, mu, ω, k, p, theta, m, Δ, part_function::Function)
    p_plus_k = sqrt(p^2 + k^2 + 2 * p * k * cos(theta))
    ...
    ff(x) = numberF(T, 0.0, x)
    prefactor(e, ep) = k^2 * sin(theta) * ckpk(k, p, m, e, ep, theta) / (2 * π^2)
...
function integrand_Pi_0_3(T, mu, ω, k, p, theta, m, Δ, part_function::Function)
    p_plus_k = sqrt(p^2 + k^2 + 2 * p * k * cos(theta))
    eps1_sign(sign) = eps_k(k, m, mu, Δ, sign)
    eps2_sign(sign) = eps_k(p_plus_k, m, mu, Δ, sign)
    xi1_sign(sign) = xi_k(k, m, mu, sign)
    xi2_sign(sign) = xi_k(p_plus_k, m, mu, sign)
    ff(x) = numberF(T, 0.0, x)
    prefactor(e, ep) = k^2 * sin(theta) * ckpk(k, p, m, e, ep, theta) / (π^2)
...
function integrand_Pi_1_3(T, mu, ω, k, p, theta, m, Δ, part_function::Function)
    p_plus_k = sqrt(p^2 + k^2 + 2 * p * k * cos(theta))
    eps1_sign(sign) = eps_k(k, m, mu, Δ, sign)
    eps2_sign(sign) = eps_k(p_plus_k, m, mu, Δ, sign)
    xi1_sign(sign) = xi_k(k, m, mu, sign)
    xi2_sign(sign) = xi_k(p_plus_k, m, mu, sign)
    ff(x) = numberF(T, 0.0, x)
    prefactor(e, ep) = k^2 * sin(theta) * ckpk(k, p, m, e, ep, theta) / (4 * π^2)
...
function real_part_function(x; η=1e-2)
    return x / (x^2 + η^2)
end

function imag_part_function(x; η=1e-2)
    return -η / (x^2 + η^2)
end

function Pi_1(T, mu, ω, p, m, Δ, part_function, param)
    return 0.5 * (integrate(x -> integrand_Pi_0_1(T, mu, ω, x[1], p, x[2], m, Δ, part_function), [0.0, 0.0], [param.Λ, π]))
end
function Pi_R_3(T, mu, ω, p, m, Δ, part_function, param)
    return 0.5 * (integrate(x -> integrand_Pi_0_3(T, mu, ω, x[1], p, x[2], m, Δ, part_function), [0.0, 0.0], [param.Λ, π]) +
                  integrate(x -> integrand_Pi_1_3(T, mu, ω, x[1], p, x[2], m, Δ, part_function), [0.0, 0.0], [param.Λ, π]))
end
